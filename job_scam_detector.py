        ...
        if run:
            ...
            with st.spinner("Running analysis — this takes ~10 seconds…"):
                ...
                try:
                    ...
                    # ── v7 IMPROVEMENT 3b: Positive trust score ───────────────────
                    # Strong positive infrastructure signals actively reduce the score.
                    # Previously, clean probes only avoided adding penalty — they never
                    # pushed the score down. This rewards genuinely verified companies.
                    trust_reduction = 0
                    ...
                    if cd_trust.get("identity_sources", 0) >= 2:
                        trust_reduction += 8   # Clearbit + Wikipedia/LinkedIn confirmed
                    elif cd_trust.get("identity_sources", 0) == 1:
                        trust_reduction += 3

                    if age_trust.get("status") == "established":
                        trust_reduction += 4   # domain >2 years old
                    if spf_trust.get("spf") and spf_trust.get("dmarc"):
                        trust_reduction += 3   # full email authentication configured
                    if cd_trust.get("domain_exists") and cd_trust.get("website_live"):
                        trust_reduction += 2   # real reachable website

                    # MCA confirmed separately (strongest India-specific trust signal)
                    mca_result = cd_trust.get("mca", {})
                    if mca_result.get("found") is True:
                        trust_reduction += 6

                    # Only apply trust reduction when blended is in caution/borderline zone
                    # (25–60). For clear scams (>60), positive signals don't override.
                    if 25 <= blended <= 60 and trust_reduction > 0:
                        blended = max(blended - trust_reduction, 0)
