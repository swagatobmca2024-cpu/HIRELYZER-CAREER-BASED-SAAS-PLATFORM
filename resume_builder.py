...
def _tech_pills(tech_str, color="#374151", bg="#f3f4f6", border="#d1d5db", font_size="11px"):
    """
    Converts a comma-separated tech stack string into wrapping pill badges.
    Prevents overflow in project cards by rendering each technology as an
    individual pill inside a flex-wrap container.
    Falls back gracefully to plain text if tech_str is empty.
    """
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get('title'):
            ...
            proj_html += (
                ...
                f"</div>"
                f"{_tech_pills(proj.get('tech',''), color='#4b5563', bg='#f1f5f9', border='#cbd5e1')}"
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get('title'):
            ...
            proj_html += (
                ...
                f"</div>"
                f"{_tech_pills(proj.get('tech',''), color='#374151', bg='#f3f4f6', border='#d1d5db')}"
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get('title'):
            ...
            proj_sb += (
                ...
                f"</div>"
                f"{_tech_pills(proj.get('tech',''), color='#0284c7', bg='#e0f2fe', border='#7dd3fc')}"
                f"<div style='font-size:13px;color:#374151;'>{desc}</div>"
                f"{proj_link_html}</div>"
            )
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get("title"):
            ...
            <div style='margin-bottom:16px;'>
                ...
                {_tech_pills(proj.get('tech',''), color='#374151', bg='#f3f4f6', border='#d1d5db')}
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get("title"):
            ...
            <div style='margin-bottom:14px;padding-left:16px;border-left:3px solid #3730a3;'>
                ...
                {_tech_pills(proj.get('tech',''), color='#3730a3', bg='#eef2ff', border='#a5b4fc')}
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get('title'):
            ...
            <div style='margin-bottom:14px;padding:12px 14px;background:#eff6ff;border-radius:6px;border-left:3px solid #1d4ed8;'>
                ...
                {_tech_pills(proj.get('tech',''), color='#1d4ed8', bg='#dbeafe', border='#93c5fd')}
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get('title'):
            ...
            <div style='margin-bottom:14px;padding:12px;background:#fff;border:1px solid #a7f3d0;border-radius:8px;'>
                ...
                {_tech_pills(proj.get('tech',''), color='#065f46', bg='#d1fae5', border='#6ee7b7')}
                <div style='font-size:13px;color:#374151;'>{desc}</div>
                {proj_link_html}
            </div>"""
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get('title'):
            ...
            <div style='margin-bottom:14px;padding:12px;background:#fafaf9;border-radius:6px;border:1px solid #d6d3d1;border-left:3px solid #d97706;'>
                ...
                {_tech_pills(proj.get('tech',''), color='#92400e', bg='#fef3c7', border='#fcd34d')}
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get('title'):
            ...
            <div style='margin-bottom:18px;padding:12px 14px;background:#f8fafc;
                ...
                {_tech_pills(proj.get('tech',''), color=C_SECONDARY, bg='#f3f4f6', border='#d1d5db')}
                <div style='font-size:14px;color:{C_BODY};line-height:1.75;'>{desc}</div>
                {proj_link_html}
            </div>"""
    ...
    for edu in session_state.education_entries:
        if edu.get('institution'):
            ...
            edu_html_fg += (
                ...
                f"<div style='display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:4px;'>"
                f"<strong style='font-size:15px;color:#14532d;'>{edu.get('institution','')}</strong>"
                f"<span style='font-size:13px;color:#6b7280;'>{edu.get('year','')}</span>"
                f"</div>"
                f"<div style='font-size:14px;color:#374151;font-style:italic;font-weight:600;'>{dv}</div>"
                f"<div style='font-size:13px;color:#6b7280;'>{edu.get('details','')}</div></div>"
            )
    ...
    for edu in session_state.education_entries:
        if edu.get('institution') or edu.get('degree'):
            ...
            <div style='margin-bottom:12px;'>
                ...
                <div style='font-size:13px;color:#6b7280;margin-top:2px;'>{edu.get('details','')}</div>
            </div>"""
    ...
    for exp in session_state.experience_entries:
        if exp.get('company') or exp.get('title'):
            ...
            <div style='margin-bottom:18px;padding:12px 16px;background:#fafafa;
                ...
                <div style='font-size:14px;color:{C_MUTED};font-style:italic;margin:3px 0 8px;'>{exp.get('title','')}</div>
                {desc}
            </div>"""
    ...
    for edu in session_state.education_entries:
        if edu.get('institution') or edu.get('degree'):
            ...
            <div style='margin-bottom:12px;padding:10px 14px;background:#fafafa;
                ...
                <div style='font-size:14px;color:{C_MUTED};font-style:italic;margin-top:3px;'>{dv}</div>
                <div style='font-size:13px;color:{C_MUTED};margin-top:2px;'>{edu.get('details','')}</div>
            </div>"""
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get('title'):
            ...
            <div style='margin-bottom:14px;padding:12px 16px;background:#fafafa;
                ...
                {_tech_pills(proj.get('tech',''), color=C_ACC, bg='#f3f4f6', border='#d1d5db')}
                {desc}{lnk_html}
            </div>"""
...
<div>
  <div style='text-align:center;margin-bottom:10px;'>
    {fixed_img}
    <h1 style='font-size:32px;font-weight:800;color:{C_HEAD};letter-spacing:0.5px;'>{session_state.get('name','')}</h1>
    {job_title_line}
    <p style='font-size:13px;color:{C_MUTED};margin-top:8px;'>{contact_line}</p>
  </div>
    ...
    for exp in session_state.experience_entries:
        if exp.get('company') or exp.get('title'):
            ...
            <div style='margin-bottom:18px;'>
                ...
                <div style='font-size:14px;color:{C_ACC};font-weight:700;margin:3px 0 6px;'>{exp.get('title','')}</div>
                {desc}
            </div>
    ...
    for edu in session_state.education_entries:
        if edu.get('institution') or edu.get('degree'):
            ...
            <div style='margin-bottom:14px;'>
                ...
                <div style='font-size:14px;color:{C_ACC};font-style:italic;font-weight:600;margin-top:3px;'>{dv}</div>
                <div style='font-size:13px;color:{C_MUTED};margin-top:2px;'>{edu.get('details','')}</div>
            </div>"""
...
<div>
  <div style='text-align:center;margin-bottom:10px;'>
    {fixed_img}
    <h1 style='font-size:32px;font-weight:700;color:{C_HEAD};letter-spacing:0.5px;'>{session_state.get('name','')}</h1>
    {job_title_line}
    <p style='font-size:13px;color:{C_MUTED};margin-top:8px;'>{contact_line}</p>
  </div>
    ...
    for exp in session_state.experience_entries:
        if exp.get('company') or exp.get('title'):
            ...
            <div style='margin-bottom:18px;'>
                ...
                <div style='font-size:14px;color:{C_MUTED};font-weight:700;margin:3px 0 6px;letter-spacing:0.3px;'>{exp.get('title','')}</div>
                {desc}
...
<div>
  <div style='text-align:center;margin-bottom:10px;'>
    {fixed_img}
    <h1 style='font-size:32px;font-weight:800;color:{C_HEAD};letter-spacing:1px;'>{session_state.get('name','')}</h1>
    {job_title_line}
    <p style='font-size:13px;color:{C_MUTED};margin-top:8px;'>{contact_line}</p>
  </div>
  <hr style='border:none;border-top:3px solid {C_ACC};margin:16px 0 24px;'>
    ...
    for idx, proj in enumerate(session_state.project_entries):
        if proj.get('title'):
            ...
            <div style='margin-bottom:14px;padding:10px 16px;background:{C_SOFT};border-radius:6px;border-left:3px solid {C_ACC};'>
                ...
                {_tech_pills(proj.get('tech',''), color=C_ACC, bg='#fdf2f8', border='#f0abfc')}
                {desc}{lnk_html}
            </div>"""
