...
"""
    ...
    # Section 5: My Progress 📊
        ...
        if df.empty:
            ...
            # =====================================================
            # SECTION J — ADVANCED INTERVIEW RECORDS DASHBOARD
            # =====================================================
            st.markdown("---")
            st.markdown("### 📋 All Interview Records — Advanced Dashboard")
            st.caption("Every session at a glance. Filter, sort, and explore your full interview history in detail.")

            # ── Inject dashboard CSS ──
            st.markdown("""
            <style>
            .rec-dashboard-wrap {
                background: linear-gradient(160deg, rgba(0,195,255,0.04) 0%, rgba(0,20,40,0.7) 100%);
                border: 1px solid rgba(0,195,255,0.2);
                border-radius: 16px;
                padding: 0 0 8px 0;
                margin-top: 8px;
            ...
            .rec-table thead tr {
                background: linear-gradient(90deg, rgba(0,195,255,0.18) 0%, rgba(0,100,180,0.18) 100%);
            ...
            .diff-easy   { background: rgba(76,175,80,0.18);  color: #81c784; border: 1px solid rgba(76,175,80,0.35); }
            .diff-medium { background: rgba(255,152,0,0.18);  color: #ffb74d; border: 1px solid rgba(255,152,0,0.35); }
            .diff-hard   { background: rgba(244,67,54,0.18);  color: #ef9a9a; border: 1px solid rgba(244,67,54,0.35); }
            .diff-unknown{ background: rgba(150,150,150,0.12);color: #aaa;    border: 1px solid rgba(150,150,150,0.25); }
            .rec-skill-mini {
                display: flex;
                flex-direction: column;
                gap: 3px;
            ...
            .skill-k { background: linear-gradient(90deg,#00c3ff,#0080cc); }
            .skill-c { background: linear-gradient(90deg,#a78bfa,#7c3aed); }
            .skill-r { background: linear-gradient(90deg,#34d399,#059669); }
            .rec-date-text {
            ...
            # ── Filter controls ──
            _filt_cols = st.columns([1, 1, 1, 1])
            with _filt_cols[0]:
                _roles_avail = ["All Roles"] + sorted(df['role'].dropna().unique().tolist()) if 'role' in df.columns else ["All Roles"]
                _filt_role = st.selectbox("Filter by Role", _roles_avail, key="rec_filt_role")
            with _filt_cols[1]:
                _diff_avail = ["All Levels"] + sorted(df['difficulty'].dropna().unique().tolist()) if 'difficulty' in df.columns else ["All Levels"]
                _filt_diff = st.selectbox("Filter by Level", _diff_avail, key="rec_filt_diff")
            with _filt_cols[2]:
                _score_min, _score_max = 0.0, 10.0
                if 'avg_score' in df.columns and df['avg_score'].notna().any():
                    _score_min = float(df['avg_score'].min())
                    _score_max = float(df['avg_score'].max())
                _filt_score = st.slider("Min Score", 0.0, 10.0, _score_min, 0.5, key="rec_filt_score")
            with _filt_cols[3]:
                _sort_opts = {"Latest First": ("id", False), "Oldest First": ("id", True),
                              "Highest Score": ("avg_score", False), "Lowest Score": ("avg_score", True)}
                _filt_sort = st.selectbox("Sort By", list(_sort_opts.keys()), key="rec_filt_sort")

            # ── Apply filters ──
            _rec_df = df.copy()
            if _filt_role != "All Roles" and 'role' in _rec_df.columns:
                _rec_df = _rec_df[_rec_df['role'] == _filt_role]
            if _filt_diff != "All Levels" and 'difficulty' in _rec_df.columns:
                _rec_df = _rec_df[_rec_df['difficulty'] == _filt_diff]
            if 'avg_score' in _rec_df.columns:
                _rec_df = _rec_df[_rec_df['avg_score'].fillna(0) >= _filt_score]
            _sort_col, _sort_asc = _sort_opts[_filt_sort]
            if _sort_col in _rec_df.columns:
                _rec_df = _rec_df.sort_values(_sort_col, ascending=_sort_asc)
            _rec_df = _rec_df.reset_index(drop=True)

            # ── Summary strip ──
            if not _rec_df.empty and 'avg_score' in _rec_df.columns:
                _s1, _s2, _s3, _s4 = st.columns(4)
                with _s1:
                    st.markdown(f"""<div class="neon-metric-card" style="margin:4px 0;">
                        <div class="neon-metric-label">Showing</div>
                        <div class="neon-metric-value">{len(_rec_df)} records</div></div>""", unsafe_allow_html=True)
                with _s2:
                    _avg_v = _rec_df['avg_score'].mean()
                    st.markdown(f"""<div class="neon-metric-card" style="margin:4px 0;">
                        <div class="neon-metric-label">Avg Score (filtered)</div>
                        <div class="neon-metric-value">{_avg_v:.2f}/10</div></div>""", unsafe_allow_html=True)
                with _s3:
                    _best_v = _rec_df['avg_score'].max()
                    st.markdown(f"""<div class="neon-metric-card" style="margin:4px 0;">
                        <div class="neon-metric-label">Best in Filter</div>
                        <div class="neon-metric-value">{_best_v:.2f}/10</div></div>""", unsafe_allow_html=True)
                with _s4:
                    _qs_v = int(_rec_df['total_questions'].fillna(0).sum()) if 'total_questions' in _rec_df.columns else 0
                    st.markdown(f"""<div class="neon-metric-card" style="margin:4px 0;">
                        <div class="neon-metric-label">Questions (filtered)</div>
                        <div class="neon-metric-value">{_qs_v}</div></div>""", unsafe_allow_html=True)

            # ── Build HTML table ──
            def _score_color(v):
                if v >= 8:   return "#00e676"
                if v >= 6.5: return "#66bb6a"
                if v >= 5:   return "#ffb74d"
            ...
            def _bar_html(pct, color, css_class=""):
                return f'<div class="rec-score-bar-bg"><div class="rec-score-bar-fill {css_class}" style="width:{min(pct,100):.1f}%;background:{color};"></div></div>'

            if _rec_df.empty:
                st.markdown('<div class="rec-dashboard-wrap"><p class="rec-no-data">No records match your filters.</p></div>', unsafe_allow_html=True)
            else:
                rows_html = ""
                for _ri, _row in _rec_df.iterrows():
                    _score    = float(_row.get('avg_score', 0) or 0)
                    _wscore   = float(_row.get('weighted_score', _score) or _score)
                    _know     = float(_row.get('knowledge_avg', _score) or _score)
                    _comm     = float(_row.get('communication_avg', _score) or _score)
                    _relv     = float(_row.get('relevance_avg', _score) or _score)
                    _diff     = str(_row.get('difficulty', 'Unknown') or 'Unknown')
                    _role_v   = str(_row.get('role', '—') or '—')
                    _domain_v = str(_row.get('domain', '—') or '—')
                    _date_v   = str(_row.get('completed_on', '—') or '—')[:16]
                    _dur_s    = _row.get('duration_seconds', None)
                    _dur_txt  = f"{int(float(_dur_s)//60)}m {int(float(_dur_s)%60)}s" if _dur_s and str(_dur_s) not in ('nan','None','') else "—"
                    _qs_v2    = int(float(_row.get('total_questions', 0) or 0))
                    _behav    = str(_row.get('behavior_class', '') or '')
                    _rec_num  = int(_row.get('id', _ri+1) if 'id' in _row.index else _ri+1)
                    _sc_color = _score_color(_score)
                    _sc_pct   = (_score / 10) * 100
                    _ws_pct   = (_wscore / 10) * 100
                    _ws_color = _score_color(_wscore)

                    _skill_block = f"""
                    <div class="rec-skill-mini">
                        <div class="rec-skill-row">K
                            <div class="rec-skill-bar-bg"><div class="rec-skill-bar-fill skill-k" style="width:{(_know/10)*100:.0f}%"></div></div>
                            <span style="min-width:22px;text-align:right;font-size:10px;color:#7dd9ff">{_know:.1f}</span>
                        </div>
                        <div class="rec-skill-row">C
                            <div class="rec-skill-bar-bg"><div class="rec-skill-bar-fill skill-c" style="width:{(_comm/10)*100:.0f}%"></div></div>
                            <span style="min-width:22px;text-align:right;font-size:10px;color:#c4b5fd">{_comm:.1f}</span>
                        </div>
                        <div class="rec-skill-row">R
                            <div class="rec-skill-bar-bg"><div class="rec-skill-bar-fill skill-r" style="width:{(_relv/10)*100:.0f}%"></div></div>
                            <span style="min-width:22px;text-align:right;font-size:10px;color:#6ee7b7">{_relv:.1f}</span>
                        </div>
                    </div>"""

                    _behav_html = f'<span class="rec-behavior-chip">{_behav}</span>' if _behav else '<span style="color:rgba(255,255,255,0.2);font-size:11px">—</span>'

                    rows_html += f"""
                    <tr>
                        <td class="rec-idx">{_rec_num}</td>
                        <td><span class="rec-role-pill">{_role_v}</span></td>
                        <td><span class="rec-domain-text">{_domain_v}</span></td>
                        <td>
                            <div class="rec-score-bar-wrap">
                                {_bar_html(_sc_pct, _sc_color)}
                                <span class="rec-score-val" style="color:{_sc_color}">{_score:.2f}</span>
                            </div>
                        </td>
                        <td>
                            <div class="rec-score-bar-wrap">
                                {_bar_html(_ws_pct, _ws_color)}
                                <span class="rec-score-val" style="color:{_ws_color}">{_wscore:.2f}</span>
                            </div>
                        </td>
                        <td>{_skill_block}</td>
                        <td><span class="rec-diff-badge {_diff_class(_diff)}">{_diff}</span></td>
                        <td><span class="rec-qs-badge">{_qs_v2}</span></td>
                        <td><span class="rec-dur-text">{_dur_txt}</span></td>
                        <td>{_behav_html}</td>
                        <td><span class="rec-date-text">{_date_v}</span></td>
                    </tr>"""

                table_html = f"""
                <div class="rec-dashboard-wrap">
                ...
                <p style="color:rgba(255,255,255,0.3);font-size:11px;margin:6px 0 0 2px;">
                    K = Knowledge &nbsp;|&nbsp; C = Communication &nbsp;|&nbsp; R = Relevance &nbsp;|&nbsp; Adj. Score accounts for difficulty multiplier
                </p>"""
