...
"""
    ...
    # Section 4: UPDATED AI Interview Coach 🤖 with Resume-Based Interviewing
        ...
        if st.session_state.resume_file is not None:
            ...
            # Determine the domain index from session_state so we can read it
            # before the widget renders (needed to compute the role list below).
            _saved_domain = st.session_state.get("_sel_domain_val", _domain_options[0])
            _domain_idx = _domain_options.index(_saved_domain) if _saved_domain in _domain_options else 0
            ...
            # Persist the chosen domain so we can detect a real change next run
            st.session_state["_sel_domain_val"] = selected_domain

            # ── Role list — always derived from the current domain ────────────
            # When the domain changes we force the role index back to 0 by
            # tracking the last domain that was active when a role was chosen.
            # No key deletion → no extra rerun, no page flash.
            with col2:
                if selected_domain:
                    roles = list(COURSES_BY_CATEGORY[selected_domain].keys())

                    # If domain just changed, reset stored role to first option
                    _last_domain_for_role = st.session_state.get("_last_domain_for_role", selected_domain)
                    if _last_domain_for_role != selected_domain:
                        st.session_state["_sel_role_val"] = roles[0]
                        st.session_state["_last_domain_for_role"] = selected_domain

                    # Compute safe index for current role inside current domain
                    _saved_role = st.session_state.get("_sel_role_val", roles[0])
                    _role_idx = roles.index(_saved_role) if _saved_role in roles else 0
                    ...
                    # Persist chosen role
                    st.session_state["_sel_role_val"] = selected_role
                    st.session_state["_last_domain_for_role"] = selected_domain
                else:
                    selected_role = None
        ...
        if selected_domain and selected_role:
            ...
            if not st.session_state.dynamic_interview_started:
                ...
                # Cache the DB call so it does not re-fire on every widget interaction
                _username_wm = st.session_state.get("username", "Guest")
                _wm_cache_key = f"_wm_cache_{_username_wm}"
                if _wm_cache_key not in st.session_state:
                    st.session_state[_wm_cache_key] = get_user_weakness_history(_username_wm)
                _wm = st.session_state[_wm_cache_key]
                ...
                # Persist setup widget values so changing one widget does not reset
                # the others — this eliminates the blink/flash on every selection.
                _itype_opts = ["technical", "behavioral", "mixed"]
                _idiff_opts = ["Easy", "Medium", "Hard"]
                _saved_itype = st.session_state.get("_setup_interview_type", "technical")
                _saved_idiff = st.session_state.get("_setup_interview_diff", "Medium")
                _saved_nq    = st.session_state.get("_setup_num_questions", 6)
                _saved_timer = st.session_state.get("_setup_timer_seconds", 120)
                _itype_idx = _itype_opts.index(_saved_itype) if _saved_itype in _itype_opts else 0
                _idiff_idx = _idiff_opts.index(_saved_idiff) if _saved_idiff in _idiff_opts else 1
                ...
                st.session_state["_setup_interview_diff"] = interview_difficulty

                col3, col4 = st.columns(2)
                with col3:
                    num_questions = st.slider("Number of questions:", 5, 10, _saved_nq)
                st.session_state["_setup_num_questions"] = num_questions

                with col4:
                    timer_seconds = st.slider("Time per question (seconds):", 60, 300, _saved_timer, step=30)
            ...
            # Interview in progress
                ...
                # Lock role/domain to what was set at interview start — prevents
                # wrong role showing if user touches dropdowns mid-interview
                selected_role   = st.session_state.get("interview_role",   selected_role)
                selected_domain = st.session_state.get("interview_domain", selected_domain)
