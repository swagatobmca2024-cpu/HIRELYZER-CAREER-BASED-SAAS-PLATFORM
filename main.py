...
""", unsafe_allow_html=True)
                ...
                with detail_tab2:
                    ...
                    if job_suggestions_display:
                        ...
                        items_html = ""
                        for line in lines:
                            # Skip any line that is purely a URL or emoji+URL
                            _stripped = line.strip()
                            if re.match(r'^https?://', _stripped):
                                continue
                            if re.sub(r'[\U0001F517\U0001F310\s]', '', _stripped).startswith('http'):
                                continue
                            if re.match(r'^[🔗\s]*https?://', _stripped):
                                continue
                            m = re.match(r'^\d+\.\s+\*\*(.+?)\*\*\s*[—\-]?\s*(.*)', _stripped)
                            if m:
                                title = m.group(1).strip()
                                # Aggressively strip all URLs and emoji from description
                                desc = m.group(2)
                                desc = re.sub(r'https?://\S+', '', desc)
                                desc = re.sub(r'[🔗🌐]\s*', '', desc)
                                desc = re.sub(r'[\U0001F517\U0001F310]\s*', '', desc)
